<template>
  <div class="box-border inline-flex items-center justify-center h-6 px-3 py-4 text-base leading-6 text-center duration-200 ease-in-out border border-gray-300 border-solid rounded-lg shadow-sm dark:border-gray-700">
    <!-- badge / icon -->
    ...
    <div class="badge-content" style="max-width: 400px;">
      ...
      <span v-else class="flex flex-row items-center dark:text-white">
        <slot></slot>
      </span>
    </div>
    ...
    <button v-if="copyButton" v-tooltip="'Copy to clipboard'" v-clipboard="() => copyText" v-clipboard:success="clipboardSuccessHandler" v-clipboard:error="clipboardErrorHandler">
      <FontAwesome class="ml-2 cursor-pointer dark:text-white" :icon="['fal', 'copy']" />
    </button>
  </div>
</template>
